...
- **Auto-sync при изменениях** - не требуется перезагрузка страницы для выгрузки изменений
- **Подписки на чужие списки** - возможность подписаться на общие списки (read-only)
- **JSON credentials** - простой экспорт/импорт учетных данных между устройствами

### Хранение данных

- IndexedDB для локального хранения
- Supabase для облачной синхронизации
- Timestamp tracking для разрешения конфликтов
...
1. **На первом устройстве:**
   ...
   - Выберите "☁️ Создать синхронизируемый список"
   - Введите название списка
   - Нажмите "🔑 Показать ID" - учетные данные будут скопированы в буфер обмена
