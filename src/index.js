...
console.log(`${LOG_PREFIX} Script loaded (readyState: ${document.readyState})`);
...
async function getDataNodeContent() {
  ...
  console.warn(`${LOG_PREFIX} dataNodeContent not found on page`);
  return null;
}
  ...
  for (const script of scripts) {
    if (script.textContent && script.textContent.includes('abCentral') && script.textContent.trim().startsWith('{')) {
      ...
      try {
        ...
        console.log(`${LOG_PREFIX} abCentral data:`, initData);
        ...
        if (catalogData && catalogData.length > 0) {
          ...
          console.log(`${LOG_PREFIX} First catalog item:`, catalogData[0]);
          return catalogData;
        } else {
